...
def generate_plots(N, mu, sigma2, S):
    ...
    # and a random dataset Y with normal additive error (mean mu, variance sigma²).
    X = np.random.rand(N)  # Generate random values for X between 0 and 1
    Y = X * 0 + np.random.normal(mu, np.sqrt(sigma2), N)  # Generate Y with normal error
    ...
    for _ in range(S):
        ...
        # Generate Y values with normal additive error (mean mu, variance sigma²)
        Y_sim = X_sim * 0 + np.random.normal(mu, np.sqrt(sigma2), N)
